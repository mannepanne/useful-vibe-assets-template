...
**Related reading:**
- Overview: [`.vibe/AGENTS.md`](../AGENTS.md) → "Automated PR review system"
...
2. If `.vibe/config/project-config.local.json` exists, read it too and merge its top-level keys on top of the committed file's values (local wins). A missing local file is fine — just use the committed values.
...
- **`"disabled"`** → reply with this line, substituting the invoking skill's name: *"The review system is disabled for this project (set via `prReviewMode` in `.vibe/config/project-config.json`). Not running `/<skill-name>`. To re-enable, change the flag to `\"enabled\"` in `.vibe/config/project-config.json` or `.vibe/config/project-config.local.json`."* Stop. Do not continue into the skill.
...
- **Write target.** If `.vibe/config/project-config.local.json` exists, write to *that* file (the presence of a local override file is a signal the user wants their changes kept local). Otherwise write to the committed `.vibe/config/project-config.json`.
...
## Local override — `.vibe/config/project-config.local.json`

The committed `.vibe/config/project-config.json` governs what cloners inherit. But the template repo itself (and any project where a maintainer wants to dogfood reviews while keeping a different committed default) needs a way to override locally without touching the checked-in file.

`.vibe/config/project-config.local.json` is gitignored. When present, the gate merges its top-level keys on top of the committed file's values — local wins. A typical local override contains exactly one key:

```json
{ "prReviewMode": "enabled" }
```

Commit intent stays in `.vibe/config/project-config.json`; per-clone dogfooding goes in the local file. A local override that matches the committed value is a no-op and can safely be deleted.
...
> - **yes** → I'll persist `"enabled"` to `.vibe/config/project-config.json` and run this review now
...
**After the user answers**, run the persist semantics above with the chosen value. For a `"later"` answer, do not modify any config file — the flag stays `"prompt-on-first-use"` and the pitch can be re-raised at the next review-adjacent moment.
...
- File paths changed from the `.claude/` directory to `.vibe/` directory
- Configuration format adapted for Vibe's declarative permission model
- Persistence logic preserved
- The core state machine and contract remain the same

The gate logic is designed to work with Vibe's skill system where skills are invoked via the `skill` tool rather than slash commands.
