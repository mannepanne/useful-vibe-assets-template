...
gh pr diff <N> | grep -iE -f .vibe/agents/triage-scan-patterns.txt || true
```
...
**Important exclusion:** `.vibe/**` (skill definitions, agent prompts, settings) is **NOT** LOW — changes here modify how every future review runs. Treat as MEDIUM (→ `standard`) at minimum. Tier UP to `team` if *either*:
- the PR touches more than 3 files under `.vibe/**`, OR
- the PR touches both `.vibe/agents/**` AND `.vibe/skills/**` in the same change.
